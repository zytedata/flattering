import argparse
import json
from csv_export import CSVStatsCollector, CSVExporter  # NOQA
...
if __name__ == "__main__":
    ...
    parser.add_argument('--path', metavar='path', type=str, help='the path to JSON file;', required=True)
    parser.add_argument('--outpath', metavar='output path', type=str, help='where to save CSV output;', required=True)
    parser.add_argument('-s', '-skip',
                        action='store_true',
                        help='skip columns with invalid data instead of stringifying;')
    parser.add_argument('--namedcolumnslimit', '--ncl',
    ...
    csv_exp.export_csv_full(
        items_list, args["outpath"]
    )
